$(function () {
    ...
    form.verify({
        ...
        repass: function (value, item) {
            ...
            // 注册表单中密码宽的内容 => 坑，获取密码框需要精确，得是注册表单中的密码框
            let pwd = $(".regiBox input[name=password]").val()
            // let pwd = $(".regiBox input[name=password]").val();
            // console.log(pwd);
        
            // 判断摩玛框内容是否和确认密码框的内容是否一致
            if (value !== pwd) {
                // return 后面的提示文字
                return "两次输入的密码不一致";
            }
    ...
    $("#loginForm").on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // 登录成功
                // 1、提示框 => layer.msg
                // layer.msg("登录成功，即将去台主页")
                // 2、需要将token存储到本地中(localStorge)
                // res.token 本身就是字符串，可以直接存储
                // localStorage.setItem("token",res.token)
                // 3、跳转页面操作
                // location.href = "index.html"
